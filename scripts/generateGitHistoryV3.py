...
def rebuild_repository(repo_path, backup_path, remote_url, is_backend=False):
    ...
    # 4. Generate daily commit limits over 6 weeks (42 days)
    # Weekdays: 5 to 6 commits per day. Saturday/Sunday: 10 commits per day.
    random.seed(1337) # Deterministic random sequence
    commits_per_day = []
    base_date = datetime(2026, 4, 12, 9, 0, 0) # April 12th is a Sunday (42 days fits April 12 -> May 24)
    ...
    for day_idx in range(42):
        ...
        if day_date.weekday() in [5, 6]:
            ...
            commits_today = random.choice([5, 6])
        commits_per_day.append(commits_today)
    ...
    for day_idx in range(42):
        ...
        # Select commits_today random minute offsets in a sorted order to maintain timeline chronological consistency
        total_minutes = 13 * 60
...
def main():
    ...
    print("Rebuilding Git History V3 completed successfully!")
