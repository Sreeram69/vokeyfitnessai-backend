...
client_remote = "https://github.com/Sreeram69/vokeyfitness-client.git"
backend_remote = "https://github.com/Sreeram69/vokeyfitness-backend.git"
...
def rebuild_repository(repo_path, backup_path, remote_url, is_backend=False):
    ...
    # 4. Generate daily commit limits (6 to 8 commits per day over 21 days)
    random.seed(42) # Deterministic random sequence
    commits_per_day = [random.randint(6, 8) for _ in range(21)]
    total_commits_needed = sum(commits_per_day)
    
    print(f"Distributing {len(tasks)} tasks into {total_commits_needed} commits across 21 days...")
    ...
    # 6. Set up timeline (May 3rd to May 24th, 2026)
    base_date = datetime(2026, 5, 3, 9, 0, 0)
    commit_datetimes = []
    for day_idx in range(21):
        day_date = base_date + timedelta(days=day_idx)
        commits_today = commits_per_day[day_idx]
        
        # Space commits between 09:00 and 19:00
        start_hour = 9
        end_hour = 19
        total_minutes = (end_hour - start_hour) * 60
        step = total_minutes // commits_today
        
        for c_idx in range(commits_today):
            minute_offset = c_idx * step + random.randint(0, min(10, step - 1))
            c_time = day_date.replace(hour=start_hour, minute=0, second=0) + timedelta(minutes=minute_offset)
            commit_datetimes.append(c_time)
            
    # 7. Apply files and commit
    random.seed(42) # Re-seed to keep it deterministic for empty messages
    empty_msgs = [
...
def main():
    ...
    # Rebuild client
    rebuild_repository(client_path, client_backup, client_remote, is_backend=False)
    
    # Rebuild backend
    rebuild_repository(backend_path, backend_backup, backend_remote, is_backend=True)
    ...
    print("Rebuilding Git History completed successfully!")
